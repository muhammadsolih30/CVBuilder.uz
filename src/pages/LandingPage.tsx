...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20">
        ...
        >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            ...
            <Button
              ...
              size="sm"
              className="gradient-primary text-white h-9 px-3 sm:px-4 text-xs sm:text-sm font-semibold shadow-md hover:shadow-lg transition-all"
            >
              <span>CV Yaratish</span>
              <ArrowRight className="w-3.5 h-3.5 ml-1 sm:ml-1.5" />
            </Button>
            
            <div className="flex items-center gap-0.5 sm:gap-1 ml-0.5 sm:ml-2 border-l border-border pl-1 sm:pl-2">
              <LanguageSwitcher />
              <ThemeToggle />
            </div>
            ...
            <Button
              variant="ghost"
              size="icon"
              className="md:hidden h-9 w-9 p-0 text-muted-foreground hover:text-foreground ml-0.5"
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden border-t border-border bg-card/95 backdrop-blur-xl px-5 py-4 overflow-hidden shadow-2xl space-y-3"
            >
              <div className="flex flex-col gap-1 text-sm font-medium">
                <a
                  href="#templates"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-muted text-foreground transition-colors"
                >
                  <Layers className="w-4 h-4 text-primary" />
                  <span>Shablonlar (1000+)</span>
                </a>
                <a
                  href="#features"
                  onClick={() => setMobileMenuOpen(false)}
                ...
                >
                  ...
                  <span>Foydalanuvchilar Fikrlari</span>
                </a>
                <a
                  href="#faq"
                  onClick={() => setMobileMenuOpen(false)}
              ...
              <div className="pt-2 border-t border-border/60 flex flex-col gap-2">
                <Button
                  ...
                  className="w-full h-10 flex items-center justify-center gap-2 border-primary/30 text-primary hover:bg-primary/5 font-semibold text-xs"
                >
                  <Sparkles className="w-4 h-4" />
                  <span>Sohamga mos shablon topish (Smart)</span>
                </Button>
                <Button
                  onClick={() => {
                    setMobileMenuOpen(false);
                  ...
                  size="sm"
                  className="w-full gradient-primary text-white h-10 font-bold text-xs shadow-md"
                >
                  <span>CV Yaratishni Boshlash</span>
      ...
      <main>
        {/* ─── Hero Sektsiya ──────────────────────────────── */}
        <section className="relative overflow-hidden pt-10 pb-16 sm:pt-20 sm:pb-28 px-4">
          ...
          <div className="max-w-5xl mx-auto text-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-primary/10 text-primary text-[11px] sm:text-sm font-semibold mb-6 border border-primary/20 shadow-sm max-w-full text-center">
                <Sparkles className="w-3.5 h-3.5 text-yellow-500 flex-shrink-0 animate-spin" style={{ animationDuration: '4s' }} />
                <span className="leading-snug">1000+ Professional Shablonlar • 100% Bepul • ATS 99%</span>
              </div>

              <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-[1.14] tracking-tight mb-5 sm:mb-6 text-foreground">
                {t('hero.title')}
              </h1>

              <p className="text-sm sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8 sm:mb-10 leading-relaxed px-2">
                {t('hero.description')}
              </p>

              {/* Asosiy tugmalar guruhi */}
              <div className="flex flex-col sm:flex-row gap-3 justify-center mb-10 sm:mb-12 max-w-lg mx-auto w-full px-2">
                <Button
                  size="lg"
                  className="gradient-primary text-white text-sm sm:text-base h-12 sm:h-13 px-8 rounded-xl shadow-xl hover:shadow-2xl transition-all font-bold group w-full sm:w-auto"
                  onClick={() => navigate("/builder")}
                >
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className="text-sm sm:text-base h-12 sm:h-13 px-6 rounded-xl border-2 hover:bg-muted font-semibold gap-2 w-full sm:w-auto"
                  onClick={() => setIsRecommenderOpen(true)}
                >
              ...
              {/* Trust Badges */}
              <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center sm:gap-x-6 text-xs sm:text-sm text-muted-foreground font-medium max-w-md sm:max-w-none mx-auto">
                <span className="flex items-center gap-1.5 p-2 sm:p-0 bg-card/60 sm:bg-transparent rounded-lg border sm:border-0 justify-center sm:justify-start">
                  <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" /> {t("landing.badges.free")}
                </span>
                <span className="flex items-center gap-1.5 p-2 sm:p-0 bg-card/60 sm:bg-transparent rounded-lg border sm:border-0 justify-center sm:justify-start">
                  <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" /> {t("landing.badges.noRegister")}
                </span>
                <span className="flex items-center gap-1.5 p-2 sm:p-0 bg-card/60 sm:bg-transparent rounded-lg border sm:border-0 justify-center sm:justify-start">
                  <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" /> {t("landing.badges.ats")}
                </span>
                <span className="flex items-center gap-1.5 p-2 sm:p-0 bg-card/60 sm:bg-transparent rounded-lg border sm:border-0 justify-center sm:justify-start">
                  <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" /> {t("landing.badges.formats")}
                </span>
              </div>
            </motion.div>
              ...
              {filteredCatalog.map((item) => (
                ...
                >
                  ...
                  <div className="p-3.5 space-y-2">
                    ...
                    >
                      <span className="hidden sm:inline">Shu shablon bilan boshlash</span>
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
              <div className="overflow-x-auto scrollbar-none">
                <table className="w-full text-left text-xs sm:text-sm min-w-[480px]">
                  <thead>
                    <tr className="border-b border-border bg-muted/40 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-muted-foreground">
                      <th className="p-3 sm:p-4">Xususiyat</th>
                      <th className="p-3 sm:p-4 text-center text-primary font-extrabold">CVBuilder.uz</th>
                      <th className="p-3 sm:p-4 text-center">MS Word</th>
                      <th className="p-3 sm:p-4 text-center">Oddiy Canva</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {comparisonData.map((item, idx) => (
                      <tr key={idx} className="hover:bg-muted/10 transition-colors">
                        <td className="p-3 sm:p-4 font-medium text-foreground">{item.feature}</td>
                        <td className="p-3 sm:p-4 text-center">
                          <CheckCircle className="w-4 sm:w-5 h-4 sm:h-5 text-emerald-500 mx-auto" />
                        </td>
                        <td className="p-3 sm:p-4 text-center">
                          {item.word ? (
                            <CheckCircle className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-muted-foreground mx-auto" />
                          ) : (
