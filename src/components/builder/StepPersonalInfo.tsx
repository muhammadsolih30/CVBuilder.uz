...
import { User, Phone, Mail, MapPin, Linkedin, Send } from 'lucide-react';
...
const fields = [
  { key: 'fullName', label: 'To\'liq ism', icon: User, placeholder: 'Alisher Navoiy', required: true },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Asosiy kontakt ma'lumotlaringizni kiriting</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        <label className="cursor-pointer group">
          <div className="w-20 h-20 rounded-full bg-muted border-2 border-dashed border-border group-hover:border-primary transition-colors flex items-center justify-center overflow-hidden">
            {data.photo ? (
              <img src={data.photo} alt="Rasm" className="w-full h-full object-cover" />
            ) : (
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Ixtiyoriy • JPG, PNG</p>
        </div>
      </div>
      ...
      <div>
        ...
        <Textarea
          ...
          placeholder="O'zingiz haqingizda qisqacha yozing. Masalan: 5 yillik tajribaga ega marketing mutaxassisi..."
          rows={4}
        />
        <p className="text-xs text-muted-foreground mt-1">
          💡 HR tavsiyasi: Natijalaringizni raqamlar bilan yozing (masalan: "Savdoni 30% oshirdim")
        </p>
      </div>
    </div>
  );
